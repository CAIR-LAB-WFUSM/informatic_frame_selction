...
def _work(process_id, model,  dataset_positive, dataset_negative, video_folder, args):
    ...
    with cuda.device(process_id):
        ...
        for key in sorted_keys:
            ...
            area = np.sum(final_mask)/(224*224)
            areas.append((key, area))
            print(f'image_name:{key}, area:{area}')

        # Save the scores to a CSV file
        output_file = os.path.join(video_folder, 'areas.csv')
        df = pd.DataFrame(areas, columns=['img', 'area'])
