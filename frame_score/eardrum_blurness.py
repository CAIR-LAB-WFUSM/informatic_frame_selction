    ...
    with cuda.device(process_id):
        ...
        for idx_loader, data_loader in enumerate([data_loader_pos, data_loader_neg]):
            for iter, pack in enumerate(data_loader):
                ...
                if idx_loader == 0:
                    highres_cam_pos[img_name]=highres_cam.cpu().numpy()
                    # Store the resized image in the dictionary
                    org_img_dict[img_name] = resized_org_img_tensor.cpu().numpy()
                else:
                    highres_cam_neg[img_name]=highres_cam.cpu().numpy()


        blurriness_list = []
        sorted_keys = sorted(highres_cam_pos.keys(), key=lambda x: int(x.split('.')[0]))
        for key in sorted_keys:
            high_res_pos = highres_cam_pos[key]
            high_res_neg = highres_cam_neg[key]

            high_res_pos = np.squeeze(high_res_pos)  # Remove singleton dimension
            high_res_neg = np.squeeze(high_res_neg)  # Remove singleton dimension
            
            # Apply the hard threshold (0.6)
            hard_threshold_mask = (1*high_res_pos-0.1*high_res_neg-0.6 > 0).astype(np.uint8)
            
            # Retain only the largest connected component
            final_mask = get_largest_connected_component(hard_threshold_mask)
            
            #get original image
            org_img = org_img_dict[key]

            resized_image, blurriness = process_and_compute_blurriness(final_mask, org_img)


            blurriness_list.append((key, blurriness))
            print(f'image_name:{key}, blurriness:{blurriness}')

        # Save the scores to a CSV file
        output_file = os.path.join(video_folder, 'blurriness.csv')
        df = pd.DataFrame(blurriness_list, columns=['img', 'blurriness'])
        df.to_csv(output_file, index=False)
        print(f'finished {os.path.basename(video_folder)}')

        del img_single, outputs, regions
